...
class System:
    ...
    async def UpdateReconnectStatusToDb(self, reconnectTime: datetime.datetime):
        ...
        r = rel.first()
        s = systemConfiguration(IsConnect=True, DisconnectTime=r['DisconnectTime'], ReconnectTime=reconnectTime,
                                IsSync=r['IsSync'])
    ...
    async def __pushDataToCloud(self, referenceTime: datetime.datetime, dt: systemConfiguration):
        ...
        rel = self.__db.Services.DeviceAttributeValueServices.FindDeviceAttributeValueWithCondition(
            or_(and_(self.__db.Table.DeviceAttributeValueTable.c.UpdateDay == updateDay,
                     self.__db.Table.DeviceAttributeValueTable.c.UpdateTime >= updateTime),
                self.__db.Table.DeviceAttributeValueTable.c.UpdateDay > updateDay))
        data = []
        for r in rel:
    ...
    async def __sendHttpRequestToPushUrl(self, data: str):
        h = Http()
        token = await self.__getToken(h)
        cookie = f"Token={token}"
